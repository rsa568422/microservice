...
public interface SchedulerMyBatisRepository {
    ...
    @Select("SELECT year, month, day, task FROM schedulers WHERE year = #{year} AND month = #{month}")
    List<SchedulerEntity> findByYearAndMonth(@Param("year") int year, @Param("month") int month);
}
